...
class Involution2d(nn.Module):
    """This class implements the 2d involution proposed in: https://arxiv.org/pdf/2103.06255.pdf Implement:
    ...
    def __init__(
        ...
        in_channels: int,
        out_channels: int,
        sigma_mapping: nn.Module | None = None,
        kernel_size: int | tuple[int, int] = (7, 7),
        stride: int | tuple[int, int] = (1, 1),
        groups: int = 1,
        reduce_ratio: int = 1,
        dilation: int | tuple[int, int] = (1, 1),
        padding: int | tuple[int, int] = (3, 3),
        bias: bool = False,
    ...
    def __repr__(self) -> str:
        ...
        """
        return (
            f"{self.__class__.__name__}({self.in_channels}, {self.out_channels}, kernel_size=({self.kernel_size[0]}, {self.kernel_size[1]}), stride=({self.stride[0]}, {self.stride[1]}), padding=({self.padding[0]}, {self.padding[1]}), "
            f"groups={self.groups}, reduce_ratio={self.reduce_mapping}, dilation=({self.dilation[0]}, {self.dilation[1]}), bias={self.bias}, sigma_mapping={self.sigma_mapping!s})"
        )

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        """
        Forward pass
        :param inputs: (torch.Tensor) Input tensor of the shape [batch size, in channels, height, width]
        ...
        assert inputs.ndimension() == 4, (
            f"Input tensor to involution must be 4d but {inputs.ndimension()}d tensor is given"
        )
        ...
        input_unfolded = self.unfold(self.initial_mapping(inputs))
        input_unfolded = input_unfolded.view(
            batch_size,
            self.groups,
        ...
        # Generate kernel
        kernel = self.span_mapping(self.sigma_mapping(self.reduce_mapping(self.o_mapping(inputs))))
        kernel = kernel.view(
            batch_size, self.groups, self.kernel_size[0] * self.kernel_size[1], kernel.shape[-2], kernel.shape[-1]
        ).unsqueeze(dim=2)
...
class Involution(nn.Module):
    ...
    def __init__(self, c1, c2, k=7, s=1, p=3, g=1, d=1, act=True):
        ...
        """
        super().__init__()
        self.conv = Involution2d(
            in_channels=c1, out_channels=c2, kernel_size=k, stride=s, padding=autopad(k, p, d), groups=g, dilation=d
        )
...
class ConvInvolution(nn.Module):
    """Convolutional module with involution."""

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        """Initialize ConvInvolution layer with given parameters."""
        super().__init__()
